...
def run_extraction_pipeline(document_text, api_key, status_container):
    ...
    status_container.write("1/3: 전역 요약 생성 중...")
    try:
...
5.  The `end_index` of an element MUST extend to the character right before the `start_index` of the NEXT element.
6.  Map the Thai header to the `type` field using these exact rules: 'ภาค' -> 'book', 'ลักษณะ' -> 'part', 'หมวด' -> 'chapter', 'ส่วน' -> 'section', 'มาตรา' -> 'article'.
