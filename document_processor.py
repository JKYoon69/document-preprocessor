...
def build_tree(flat_list):
    ...
    def finalize_structure(node):
        if "children" in node:
            # Map children to appropriate keys based on type
            ...
            # Recursively finalize children in 'sections'
            for section in node["sections"]:
                finalize_structure(section)
            
            # Clean up empty lists and temporary keys
            if not node["sections"]:
                del node["sections"]
            if not node["articles"]:
                del node["articles"]
            del node["children"]
        
        # Clean up temporary keys from all nodes
        if "level" in node:
            del node["level"]
        if "global_start" in node:
            del node["global_start"]
        if "global_end" in node:
            del node["global_end"]
