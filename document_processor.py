...
# FINAL STAGE: Builds the hierarchical tree from the flat list.
def build_tree(flat_list):
    ...
    # Filter out descriptive titles that are not actual structural elements
    # and assign hierarchy levels.
    cleaned_list = []
    for node in flat_list:
        title = node.get("title", "")
        # Keep only nodes that start with the standard Thai legal prefixes
        if title.startswith(('ภาค', 'ลักษณะ', 'หมวด', 'ส่วน', 'มาตรา')) or node.get("type") == "preamble":
             # Fix the preamble title
            if node.get("type") == "preamble":
                node["title"] = "Preamble"
    ...
    # Function to recursively finalize the structure into the desired output format
    def finalize_structure(node):
        # Rename 'children' to a more descriptive key based on child types
        if "children" in node:
            child_types = set(child.get("type") for child in node["children"])
            if "section" in child_types or "part" in child_types or "chapter" in child_types:
                 node["sections"] = node.pop("children")
                 for section in node["sections"]:
                     finalize_structure(section)
            else:
                 node["articles"] = node.pop("children")
...
def summarize_nodes_recursively(node, model, global_summary, status_container):
    ...
    # After children are summarized, summarize the current node
...
def run_final_pipeline(document_text, api_key, status_container):
    ...
    # 2. Chunking and Structure Extraction
...
{text_chunk}"""
    ...
    # 3. Post-processing
    status_container.write("3/5: Post-processing and cleaning data...")
    if not all_headers:
        raise ValueError("LLM did not extract any structure.")
    ...
    status_container.write("5/5: Generating summaries for all nodes...")
